...
def _sdpa_bwd_dv_kernel(
    ...
    for m0 in range(0, Tq, BLOCK_M):
        ...
        q_ptrs = q_ptrs_base + (m0 + offs_m)[:, None] * stride_qm + offs_d[None, :] * stride_qk
        q = tl.load(q_ptrs).to(tl.float32)
        q_mask = ((m0 + offs_m)[:, None] < Tq).to(tl.float32) * (offs_d[None, :] < D).to(tl.float32)
        q *= q_mask
        ...
        k_mask = (offs_n[:, None] < Tk).to(tl.float32) * (offs_d[None, :] < D).to(tl.float32)
        k *= k_mask

        # scores and P
        scores = tl.dot(q, tl.trans(k), out_dtype=tl.float32, input_precision="ieee") * scale  # (BLOCK_M, BLOCK_N)
        m_block = tl.load(m_ptrs_base + (m0 + offs_m) * stride_mt).to(tl.float32)
        l_block = tl.load(l_ptrs_base + (m0 + offs_m) * stride_lt).to(tl.float32)
        m_mask = (m0 + offs_m < Tq).to(tl.float32)
        m_block = m_block * m_mask + (-1.0e9) * (1.0 - m_mask)
        l_block = l_block * m_mask + (1.0 - m_mask)  # rows outside range get l=1 (will be masked)
        mask_m = (m0 + offs_m < Tq).to(tl.float32)[:, None]
        mask_n = (offs_n < Tk).to(tl.float32)[None, :]
        mask_mn_f = mask_m * mask_n
        scores = scores - m_block[:, None]
        ...
        do_ptrs = do_ptrs_base + (m0 + offs_m)[:, None] * stride_dom + offs_dv[None, :] * stride_dok
        do = tl.load(do_ptrs).to(tl.float32)
        do_mask = ((m0 + offs_m)[:, None] < Tq).to(tl.float32) * (offs_dv[None, :] < D).to(tl.float32)
        do *= do_mask
...
def _sdpa_bwd_dq_kernel(
    ...
    q_ptrs = q_ptrs_base + offs_m[:, None] * stride_qm + offs_d[None, :] * stride_qk
    q = tl.load(q_ptrs).to(tl.float32)
    q_mask = (offs_m[:, None] < Tq).to(tl.float32) * (offs_d[None, :] < D).to(tl.float32)
    q *= q_mask
    valid_m = offs_m < Tq
    ...
    for n0 in range(0, Tk, BLOCK_N):
        ...
        kv_mask = ((n0 + offs_n)[:, None] < Tk).to(tl.float32) * (offs_d[None, :] < D).to(tl.float32)
        k *= kv_mask
        v *= kv_mask

        scores = tl.dot(q, tl.trans(k), out_dtype=tl.float32, input_precision="ieee") * scale  # (Mblock, Nblock)
        m_block = tl.load(m_ptrs_base + offs_m * stride_mt).to(tl.float32)
        l_block = tl.load(l_ptrs_base + offs_m * stride_lt).to(tl.float32)
        m_mask = valid_m.to(tl.float32)
        m_block = m_block * m_mask + (-1.0e9) * (1.0 - m_mask)
        l_block = l_block * m_mask + (1.0 - m_mask)
        mask_m = valid_m.to(tl.float32)[:, None]
        mask_n = ((n0 + offs_n) < Tk).to(tl.float32)[None, :]
        mask_mn_f = mask_m * mask_n
        scores = scores - m_block[:, None]
        scores = scores * mask_mn_f + (-1.0e9) * (1.0 - mask_mn_f)
        p = tl.exp(scores) * mask_mn_f
        p = p / tl.maximum(l_block[:, None], 1e-12)

        do_ptrs = do_ptrs_base + offs_m[:, None] * stride_dom + offs_d[None, :] * stride_dok
        do = tl.load(do_ptrs).to(tl.float32)
        do_mask = (offs_m[:, None] < Tq).to(tl.float32) * (offs_d[None, :] < D).to(tl.float32)
        do *= do_mask
    ...
    dq_ptrs = DQ + pid_bh * stride_dqbh + offs_m[:, None] * stride_dqm + offs_d[None, :] * stride_dqk
    tl.store(dq_ptrs, dq_acc, mask=(offs_m[:, None] < Tq) & (offs_d[None, :] < D))
