...
from theria.attention.triton_sdpa_backward import sdpa_bwd_dk
...
def test_triton_dk_matches_reference(shape, dtype):
    ...
    dq_ref, dk_ref, dv_ref = sdpa_backward_reference(q_fp, k_fp, v_fp, do_fp, scale)
    dk_tri = sdpa_bwd_dk(q_fp, k_fp, v_fp, do_fp, m, l, scale)

    assert torch.isfinite(dk_tri).all()
    torch.testing.assert_close(dk_tri.to(torch.float32), dk_ref.to(torch.float32), rtol=2e-2, atol=2e-2)
